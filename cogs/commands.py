...
class Commands(commands.Cog):
                ...
                async with self.bot.session.get(url) as response:
                    if response.status != 200:
                        return await inter.edit_original_response(
                            embed=ErrorEmbed(f"Downloading media {url} returned status code `{response.status}`")
                        )
    ...
    async def earth(self, inter: disnake.ApplicationCommandInteraction):
        ...
        # GET request using the aiohttp session
        try:
            async with self.bot.session.get(url=f"https://epic.gsfc.nasa.gov/api/natural?api_key={self.nasa_key}") as response:
                if response.status != 200:
                    return await inter.edit_original_response(
                        embed=ErrorEmbed(f"NASA API returned status code `{response.status}`")
                    )
            ...
            return await inter.edit_original_response(
                embed=ErrorEmbed(f"NASA API returned invalid data! It might be broken right now - try again later.\n```\n{err}```")
            )
