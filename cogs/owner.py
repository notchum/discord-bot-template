...
class Owner(commands.Cog):
    ...
    async def set_status(
        ...
        await inter.response.send_message(embed=SuccessEmbed(f"Changed status to\n> {type.name} {status}"))

    @owner.sub_command()
    async def download_log(self, inter: disnake.ApplicationCommandInteraction):
        """ Download the current log file. """
        return await inter.response.send_message(file=disnake.File("log/my-bot.log"), ephemeral=True)
